  ...
  return (
    <main className="min-h-screen bg-gradient-to-b from-gray-50 to-white">
      ...
      <div className="container mx-auto px-6 relative">
        {/* Profile Header Card */}
        <div className="bg-white rounded-2xl shadow-xl -mt-20 relative z-10 p-6 md:p-8">
          <div className="flex flex-col md:flex-row gap-6">
            ...
            <div className="flex-1">
              ...
              {/* Stats Row */}
              <div className="flex flex-wrap gap-6 mt-4 pb-4 border-b">
                ...
                <div className="flex items-center gap-2 text-gray-600">
                  ...
                  <span className="font-semibold">{profileViews}</span> profile views
                </div>
                <div className="flex items-center gap-2 text-gray-600">
                  <FaBookOpen className="text-amber-500" />
                  <span className="font-semibold">{projects.length}</span> publications
                </div>
                <div className="flex items-center gap-2 text-gray-600">
                  <FaHeart className="text-amber-500" />
                  <span className="font-semibold">{posts.reduce((sum, p) => sum + (p.likes || 0), 0)}</span> total likes
                </div>
              </div>
              ...
              <div className="mt-4">
                <div className="flex flex-wrap gap-3 mb-3">
                  <span className={`px-3 py-1 rounded-full text-xs font-semibold flex items-center gap-1 ${collaborationStatus === "open" ? "bg-green-100 text-green-700" : "bg-yellow-100 text-yellow-700"}`}>
                    <FaUsers /> {collaborationStatus === "open" ? "Open to Collaborate" : "Currently Busy"}
                  </span>
                  <span className="bg-purple-100 text-purple-700 px-3 py-1 rounded-full text-xs flex items-center gap-1">
                    <FaCheckCircle /> {projects.length} Problems Solved
                  </span>
                </div>
                
                {!isEditing ? (
                  <p className="text-gray-700 leading-relaxed">{user.bio || "No bio added yet. Click Edit Profile to add your research interests."}</p>
                ) : (
                  <div className="space-y-3 mt-3">
                    <textarea value={editForm.bio || ""} onChange={(e) => setEditForm({...editForm, bio: e.target.value})} className="w-full p-3 border rounded-lg" rows={3} placeholder="Tell others about your research interests..." />
                    <div className="grid md:grid-cols-2 gap-3">
                      <input type="text" value={editForm.name || ""} onChange={(e) => setEditForm({...editForm, name: e.target.value})} className="p-2 border rounded-lg" placeholder="Full Name" />
                      <input type="text" value={editForm.institution || ""} onChange={(e) => setEditForm({...editForm, institution: e.target.value})} className="p-2 border rounded-lg" placeholder="Institution" />
                      <select value={collaborationStatus} onChange={(e) => setCollaborationStatus(e.target.value)} className="p-2 border rounded-lg">
                        <option value="open">Open to Collaborate</option>
                        <option value="busy">Busy - Not Accepting</option>
                      </select>
                      <input type="text" placeholder="Research Interests (comma separated)" className="p-2 border rounded-lg" />
                    </div>
                  </div>
                )}
              </div>
              ...
              <div className="flex gap-3 mt-4">
                <button className="text-gray-500 hover:text-blue-500 transition"><FaTwitter size={20} /></button>
                <button className="text-gray-500 hover:text-blue-700 transition"><FaLinkedin size={20} /></button>
                <button className="text-gray-500 hover:text-gray-900 transition"><FaGithub size={20} /></button>
                <button className="text-gray-500 hover:text-green-600 transition"><FaGlobe size={20} /></button>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="mt-8">
          <div className="flex gap-4 border-b">
            <button onClick={() => setActiveTab("research")} className={`px-6 py-3 font-semibold transition ${activeTab === "research" ? "text-amber-500 border-b-2 border-amber-500" : "text-gray-500 hover:text-gray-700"}`}>
              📄 Research
            </button>
            <button onClick={() => setActiveTab("posts")} className={`px-6 py-3 font-semibold transition ${activeTab === "posts" ? "text-amber-500 border-b-2 border-amber-500" : "text-gray-500 hover:text-gray-700"}`}>
              💬 Activity Feed
            </button>
            <button onClick={() => setActiveTab("achievements")} className={`px-6 py-3 font-semibold transition ${activeTab === "achievements" ? "text-amber-500 border-b-2 border-amber-500" : "text-gray-500 hover:text-gray-700"}`}>
              🏆 Achievements
            </button>
          </div>
          ...
          <div className="mt-6">
            ...
            {activeTab === "research" && (
              <div className="grid md:grid-cols-2 gap-6">
                {projects.length === 0 ? (
                  <div className="bg-white rounded-xl p-12 text-center col-span-2">
                    <FaFilePdf className="text-6xl text-gray-300 mx-auto mb-4" />
                    <p className="text-gray-500">No research published yet.</p>
                  ...
                  projects.map((project: any) => (
                    <div key={project.id} className="bg-white rounded-xl shadow-lg p-6 hover:shadow-xl transition group">
                      <div className="flex justify-between items-start mb-3">
                        <h3 className="text-xl font-bold text-blue-900 group-hover:text-amber-500 transition">{project.title}</h3>
                        <span className="text-green-600 text-sm flex items-center gap-1"><FaCheckCircle /> Solved</span>
                      </div>
            ...
            {activeTab === "achievements" && (
              <div className="grid md:grid-cols-3 gap-6">
                {achievements.map((achievement, idx) => (
                  <div key={idx} className="bg-gradient-to-br from-blue-50 to-amber-50 rounded-xl p-6 text-center hover:scale-105 transition shadow-lg">
        ...
        <div className="mt-8 bg-white rounded-2xl shadow-lg p-6">
          ...
          <div className="grid md:grid-cols-2 gap-6">
            <textarea 
              rows={4} 
              className="w-full p-4 border rounded-xl focus:ring-2 focus:ring-amber-500 transition" 
        ...
        <div className="mt-8 grid grid-cols-2 md:grid-cols-4 gap-4">
          <div className="bg-gradient-to-br from-blue-900 to-blue-700 rounded-xl p-4 text-center text-white">
            <p className="text-2xl font-bold">{projects.length}</p>
            <p className="text-sm opacity-80">Publications</p>
          </div>
          <div className="bg-gradient-to-br from-amber-500 to-amber-600 rounded-xl p-4 text-center text-white">
